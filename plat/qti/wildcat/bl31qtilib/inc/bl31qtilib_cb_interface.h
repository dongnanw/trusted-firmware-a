 ...
 * @return Zero on success, error code otherwise.
 */
int bl31qtilib_cb_int_register_fatal_isr(uint32_t intnum,
					 const char *int_desc,
					 void *(*fn)(void *), void *ctx,
					 uint32_t flags, bool enable);
...
/**
 * Configure target list for an interrupt.
 *
...
/**
 * Check if image is authenticated; if so return true and update entry point,
 * else return false.
 *
 * @param[in]     image_id    Software image ID
 * @param[in,out] entrypoint  Entry point of image that is requested
 *
...
/**
 * Report a fatal error. BL31 will forward the error to QTEE which then will
 * prepare for crashdump.
 *
 * This function does not return.
 *
 * @param[in] error_code    Fatal error reason
 */
void bl31qtilib_cb_error_fatal(int error_code);

/**
 * Report a fatal error within an ISR. BL31 will forward the error to QTEE
 * only if qteed is enabled, and QTEE will prepare for crashdump.
 *
 * This function does not return.
 *
 * @param[in] error_code    Fatal error reason
...
/**
 * Report a fatal error within an ISR. BL31 will forward the error to QTEE
 * only if qteed is enabled, and QTEE will prepare for crashdump. The
...
/**
 * Dispatch a fatal error SDEI event.
 */
