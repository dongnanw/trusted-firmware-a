				  ...
				  size_t size, uint8_t attr)
{
	...
	case BL31QTILIB_MAP_S_RW_WBWA_ISH_XN:
		l_attr = MT_SECURE | MT_RW | MT_MEMORY |
			 MT_SHAREABILITY_ISH | MT_EXECUTE_NEVER;
		ret = qti_mmap_add_dynamic_region(base_va, size, l_attr);
		break;
	case BL31QTILIB_MAP_S_RW_DEVICE_ISH_XN:
		l_attr = MT_SECURE | MT_RW | MT_DEVICE |
			 MT_SHAREABILITY_ISH | MT_EXECUTE_NEVER;
		ret = qti_mmap_add_dynamic_region(base_va, size, l_attr);
		break;
	}
			       ...
			       uint32_t flags, bool enable)
{
	...
	if (ret != 0) {
		ERROR("Failed to set EL3 interrupt target %d, %s, 0x%x",
		      intnum, int_desc, target);
		goto bail;
	}
...
{
#if defined(SPD_qteed)
	return qteed_register_qtee_isr(intnum, int_desc, fn, ctx, flags,
				       enable);
#else
	return bl31qtilib_cb_int_register_isr(intnum, int_desc, fn, ctx,
					      flags, enable);
#endif
}
...
{
#if defined(SPD_qteed)
	return qteed_disable_qtee_int(intnum);
#else
 ...
 * This implementation will return a hardcoded address for COREBOOT enabled
 * targets. This is needed because the entrypoint address is not available
 * from the bootloader in COREBOOT configurations.
 */
				     ...
				     uint64_t *entrypoint)
{
	...
	for (image_idx = 0;
	     image_idx < sbl_qsee_interface_ptr->number_images;
	     image_idx++) {
		if (image_id ==
...
void bl31qtilib_cb_error_fatal(int error_code)
{
	...
	qteed_error_handler(error_code);
#else
	/* Call the default plat_error_handler if QTEED is not present */
	plat_error_handler(error_code);
#endif
}
...
{
#if defined(SPD_qteed)
	qteed_set_err_fatal_qtee(error_code);
#else
	INFO("Function: %s error %d\n", __func__, error_code);
	panic();
#endif
}
...
{
#if defined(SPD_qteed)
	qteed_set_err_fatal_with_cond_qtee(error_code, return_to_hlos);
#else
	INFO("Function: %s error %d\n", __func__, error_code);
	panic();
#endif
...
uint32_t bl31qtilib_cb_dispatch_sdei_event(void)
{
	...
	/*
	 * We want to make sure that during the event dispatch, the NS side
	 * is not interrupted due to pending secure interrupts (e.g. pending
	 * NOCs). Those will be handled after the dispatch has completed.
	 */
	...
	/*
	 * Dispatch the SDEI event. This will restore the NS context
	 * (including our modified SCR_EL3 and SPSR_EL3) and ERET to NSEL2
	 * with FIQ routing disabled and FIQ masked.
	 */
	...
	if (ret == 0) {
		 ...
		 * COMPLETE_AND_RESUME). The NS context is already restored
		 * by sdei_dispatch_event().
		 *
		 * Restore and resume Secure context. This function was called
		 * from Secure world, so we need to return to it.
		 */
